...
{   
    ...
    for (int i = 0; i < s_ftm_report_num_entries; i++) {
        ...
        if (min_rtt_raw > s_ftm_report[i].rtt || min_rtt_raw == 0) min_rtt_raw = s_ftm_report[i].rtt;
    }
...
esp_err_t ftmInit() {
    ...
    return esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_FTM_REPORT, &ftmEventHandler, NULL,&instance_any_id);
}
